...
function AuthSection({ session, loading: sessLoading, roleError, signInWithGoogle, signOut, language = 'id' }) {
  ...
  const [pendingProofUpload, setPendingProofUpload] = useState(null)
  ...
  const invoiceDisplayNumber = (invoice, index) => `${cand?.candidate_id || 'KANDIDAT'}-${Number(invoice.description?.match(/\d+/)?.[0]) || index + 1}`
  ...
  const uploadProof = async (paymentId, invoiceId, selectedFile = null) => {
    const key = paymentId ? `payment:${paymentId}` : `invoice:${invoiceId}`
    const file = selectedFile || proofFiles[key]
    if (!file) return
    setProofBusy(key)
    try {
  ...
  useEffect(() => {
    if (!turnstileToken || !pendingProofUpload) return
    const { invoiceId, file } = pendingProofUpload
    setPendingProofUpload(null)
    uploadProof(null, invoiceId, file)
  }, [turnstileToken, pendingProofUpload])
          ...
          {session && candLoading && !cand ? (
            ...
            <>
              ...
              {activeTab === 'finance' && finance?.enabled && <div className="finance-card">
                ...
                {(() => {
                  ...
                  {showPaidDetails && <div className="fee-modal-backdrop" role="presentation" onClick={() => setShowPaidDetails(false)}>
                    <section className="fee-modal" role="dialog" aria-modal="true" aria-labelledby="paid-details-title" onClick={(event) => event.stopPropagation()}>
                      <div className="fee-modal-header">
                        <div><div className="field-label">Riwayat pembayaran</div><h3 id="paid-details-title">Pembayaran yang sudah diterima</h3></div>
                        <button type="button" className="fee-modal-close" onClick={() => setShowPaidDetails(false)} aria-label="Tutup riwayat pembayaran">×</button>
                      </div>
                      <div className="fee-detail-list">
                        {(finance.payments || []).length === 0 && <p className="document-note">Belum ada pembayaran yang diterima.</p>}
                  ...
                  {invoicePreview && <div className="fee-modal-backdrop invoice-preview-backdrop" role="presentation" onClick={() => setInvoicePreview(null)}>
                    <section className="fee-modal invoice-preview-modal" role="dialog" aria-modal="true" aria-labelledby="invoice-preview-title" onClick={(event) => event.stopPropagation()}>
                      ...
                      <div className="invoice-heading"><div><span className="invoice-kicker">TAGIHAN CICILAN</span>                      <h3 id="invoice-preview-title">Invoice #{invoiceDisplayNumber(invoicePreview, (finance.invoices || []).indexOf(invoicePreview))}</h3></div><button type="button" className="fee-modal-close" onClick={() => setInvoicePreview(null)} aria-label="Tutup preview invoice">×</button></div>
                      ...
                      <div className={`invoice-status ${invoicePreview.status === 'paid' ? 'invoice-status-paid' : 'invoice-status-unpaid'}`}>Status pembayaran: <strong>{invoicePreview.status === 'paid' ? 'LUNAS' : 'BELUM LUNAS'}</strong></div>
                      <div className="invoice-actions no-print"><button type="button" className="invoice-btn" onClick={() => window.print()}>Download invoice</button><button type="button" className="invoice-btn" onClick={() => setInvoicePreview(null)}>Tutup</button></div>
                    </section>
                  </div>}
                  </>
                })()}
                ...
                {(finance || {}).invoices?.length > 0 && <div className="payment-list"><h3>Tagihan cicilan</h3><div className="payment-list-header"><span>Tagihan</span><span>Status</span><span>Aksi</span></div>{finance.invoices.map((invoice, index) => { const proof = proofFor(null, invoice.id); const key = `invoice:${invoice.id}`; const isPaid = invoice.status === 'paid'; const fileSelected = Boolean(proofFiles[key]) || pendingProofUpload?.invoiceId === invoice.id; return <article className="payment-row" key={invoice.id}><div><strong>{invoiceDisplayNumber(invoice, index)} · {money(invoice.amount)}</strong><span>{invoice.description || 'Cicilan'} · Jatuh tempo: {invoice.due_date || 'Belum ditentukan'}</span>{proof && <small className={`proof-status proof-${proof.status}`}>{isPaid || proof.status === 'accepted' ? 'Pembayaran diterima' : proof.status === 'rejected' ? `Bukti ditolak${proof.review_note ? `: ${proof.review_note}` : ''}` : 'Bukti sedang diverifikasi'}</small>}</div><span className={`payment-status ${isPaid ? 'payment-status-paid' : 'payment-status-unpaid'}`}>{isPaid ? 'Lunas' : 'Belum dibayar'}</span><div className="payment-actions"><button type="button" className="invoice-btn" onClick={() => openInvoice(invoice)}>INVOICE</button>{!isPaid && (!proof || proof.status === 'rejected') && <label className={`proof-file ${fileSelected ? 'proof-file-selected' : ''}`}><input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(event) => { const file = event.target.files?.[0] || null; setProofFiles((current) => ({ ...current, [key]: file })); if (file) { if (turnstileToken) uploadProof(null, invoice.id, file); else { setPendingProofUpload({ invoiceId: invoice.id, file }); setFinanceError('Silakan centang verifikasi Cloudflare terlebih dahulu, lalu upload akan berjalan otomatis.') } } }} />{proofBusy === key ? 'MENGUNGGAH…' : fileSelected ? 'DOKUMEN DIPILIH' : 'UPLOAD'}</label>}</div></article>})}</div>}
                {pendingProofUpload && turnstileSiteKey && <div className="turnstile-box"><Turnstile siteKey={turnstileSiteKey} options={{ action: 'finance-proof-upload', theme: 'light' }} onSuccess={handleTurnstileSuccess} onExpire={() => setTurnstileToken('')} onError={() => setTurnstileToken('')} /></div>}
                {pendingProofUpload && !turnstileSiteKey && <p className="document-note">Upload bukti belum aktif karena Turnstile belum dikonfigurasi.</p>}
              </div>}
            </>
          )}
        </Reveal>
