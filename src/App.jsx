          ...
          {session && candLoading && !cand ? (
            ...
            <>
              ...
              {activeTab === 'finance' && finance?.enabled && <div className="finance-card">
                ...
                {(() => {
                  ...
                  {invoicePreview && <div className="fee-modal-backdrop invoice-preview-backdrop" role="presentation" onClick={() => setInvoicePreview(null)}>
                    <section className="fee-modal invoice-preview-modal" role="dialog" aria-modal="true" aria-labelledby="invoice-preview-title" onClick={(event) => event.stopPropagation()}>
                      ...
                      {(() => { const proof = proofFor(null, invoicePreview.id); const key = `invoice:${invoicePreview.id}`; return (!proof || proof.status === 'rejected') && <div className="invoice-upload-box"><div className="field-label">Unggah bukti invoice</div><div className="payment-actions"><label className="proof-file"><input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(event) => setProofFiles((current) => ({ ...current, [key]: event.target.files?.[0] || null }))} />{proofFiles[key]?.name || 'Pilih bukti'}</label><button type="button" className="invoice-btn" onClick={() => uploadProof(null, invoicePreview.id)} disabled={!proofFiles[key] || proofBusy === key || !turnstileToken}>{proofBusy === key ? 'Mengunggah…' : 'Unggah bukti'}</button></div>{turnstileSiteKey && <Turnstile siteKey={turnstileSiteKey} options={{ action: 'finance-proof-upload', theme: 'light' }} onSuccess={setTurnstileToken} onExpire={() => setTurnstileToken('')} onError={() => setTurnstileToken('')} />}</div> })()}
                    </section>
                  </div>}
                  </>
                })()}
                ...
                {(finance || {}).invoices?.some((invoice) => invoice.status === 'open') && <div className="payment-list"><h3>Tagihan cicilan</h3>{finance.invoices.filter((invoice) => invoice.status === 'open').map((invoice, index) => { const proof = proofFor(null, invoice.id); return <article className="payment-row" key={invoice.id}><div><strong>{invoiceDisplayNumber(invoice, index)} · {money(invoice.amount)}</strong><span>{invoice.description || 'Cicilan'} · Jatuh tempo: {invoice.due_date || 'Belum ditentukan'}</span>{proof && <small className={`proof-status proof-${proof.status}`}>{proof.status === 'rejected' ? `Bukti ditolak${proof.review_note ? `: ${proof.review_note}` : ''}` : 'Bukti sedang diverifikasi'}</small>}</div><div className="payment-actions"><button type="button" className="invoice-btn" onClick={() => openInvoice(invoice)}>Review invoice</button></div></article>})}</div>}
