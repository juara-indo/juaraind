...
function App() {
  ...
  return (
    <main>
      ...
      {selectedInvoice && <div className="modal-backdrop invoice-backdrop" role="presentation" onClick={() => setSelectedInvoice(null)}><section className="modal invoice-modal" role="dialog" aria-modal="true" aria-labelledby="invoice-title" onClick={(event) => event.stopPropagation()}><div className="modal-header"><div><span className="eyebrow">INVOICE CICILAN</span><h2 id="invoice-title">Invoice #{String(selectedInvoice.installmentNumber).padStart(2, '0')}</h2><p>{selectedInvoice.candidate.candidate_id}</p></div><button className="icon-button no-print" aria-label="Tutup invoice" title="Tutup" onClick={() => setSelectedInvoice(null)}><Icon name="close" /></button></div><div className="invoice-meta"><div><span>Kandidat</span><strong>{selectedInvoice.candidate.full_name || 'Nama belum diisi'}</strong></div><div><span>Tanggal pembayaran</span><strong>{new Date(`${selectedInvoice.payment.payment_date}T00:00:00`).toLocaleDateString('id-ID')}</strong></div></div><div className="invoice-lines"><div><span>Jasa paspor</span><strong>{selectedInvoice.candidate.passport_by_agency ? formatCurrency(selectedInvoice.candidate.passport_fee) : '-'}</strong></div><div><span>Jasa visa</span><strong>{selectedInvoice.candidate.visa_by_agency ? formatCurrency(selectedInvoice.candidate.visa_fee) : '-'}</strong></div><div><span>Biaya keberangkatan</span><strong>{formatCurrency(selectedInvoice.candidate.departure_fee)}</strong></div><div className="invoice-total"><span>Total tagihan</span><strong>{formatCurrency(financeTotal(selectedInvoice.candidate))}</strong></div><div className="invoice-payment"><span>Cicilan ini</span><strong>{formatCurrency(selectedInvoice.payment.amount)}</strong></div><div><span>Total dibayar s.d. cicilan ini</span><strong>{formatCurrency(selectedInvoice.paidThrough)}</strong></div><div className="invoice-remaining"><span>Sisa cicilan</span><strong>{formatCurrency(selectedInvoice.remaining)}</strong></div></div>{selectedInvoice.payment.note && <p className="invoice-note">Catatan: {selectedInvoice.payment.note}</p>}<div className="invoice-actions no-print"><button className="collective-convert" onClick={() => window.print()}>Cetak invoice</button><button className="documents-button" onClick={() => setSelectedInvoice(null)}>Tutup</button></div></section></div>}
