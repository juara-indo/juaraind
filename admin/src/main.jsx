...
function App() {
  ...
  return (
    <main>
      ...
      <section className="panel"><div className="table-wrap"><table><thead><tr><th>No</th><th>Kandidat</th><th>Biodata</th><th>Kolektif</th><th>Dokumen</th></tr></thead><tbody>{candidates.map((candidate, index) => { const biodataComplete = isBiodataComplete(candidate); const types = collectiveTypes(candidate); const files = collectiveFiles[candidate.candidate_id] || {}; const collectiveDone = types.length > 0 && types.every((type) => candidate.documents.some((document) => document.document_type === type)); return <tr key={candidate.candidate_id}><td>{index + 1}</td><td className="candidate-cell"><span className="candidate-name">{candidate.full_name || 'Nama belum diisi'}</span><strong className="candidate-id">{candidate.candidate_id}</strong></td><td><button className={`documents-button biodata-button ${biodataComplete ? 'is-complete' : 'is-incomplete'}`} title={biodataComplete ? 'Biodata lengkap' : 'Biodata belum lengkap'} onClick={() => setSelectedBiodata(candidate)}>Biodata</button></td><td>{!types.length || collectiveDone ? <button className="documents-button collective-button" disabled>Tidak aktif</button> : <button className={`documents-button collective-button ${files[types[0]] ? 'is-ready' : ''}`} onClick={() => setSelectedCollectiveCandidate(candidate)}>Upload</button>}</td><td><button className="documents-button" onClick={() => setSelectedCandidate(candidate)}>Lihat dokumen <span className="document-count">({candidate.documents.length})</span></button></td></tr> })}</tbody></table></div>{!candidates.length && <div className="empty">Belum ada peserta.</div>}</section>
      {selectedCollectiveCandidate && <div className="modal-backdrop" role="presentation" onClick={() => setSelectedCollectiveCandidate(null)}><section className="modal collective-modal" role="dialog" aria-modal="true" aria-labelledby="collective-title" onClick={(event) => event.stopPropagation()}><div className="modal-header"><div><span className="eyebrow">DOKUMEN KOLEKTIF</span><h2 id="collective-title">{selectedCollectiveCandidate.full_name || 'Nama belum diisi'}</h2><p>{selectedCollectiveCandidate.candidate_id}</p></div><button className="icon-button" aria-label="Tutup upload kolektif" title="Tutup" onClick={() => setSelectedCollectiveCandidate(null)}><Icon name="close" /></button></div><p className="collective-help">Upload dokumen kolektif yang dipilih saat pendaftaran.</p><div className="collective-fields">{collectiveTypes(selectedCollectiveCandidate).map((type) => <label className="collective-field" key={type}><span>{type.toUpperCase()}</span><input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(event) => handleCollectiveFile(selectedCollectiveCandidate, type, event.target.files?.[0])} /><strong>{collectiveFiles[selectedCollectiveCandidate.candidate_id]?.[type]?.name || 'Pilih file'}</strong></label>)}</div><button className="collective-convert" disabled={!collectiveTypes(selectedCollectiveCandidate).every((type) => collectiveFiles[selectedCollectiveCandidate.candidate_id]?.[type])} onClick={async () => { await convertCollective(selectedCollectiveCandidate); setSelectedCollectiveCandidate(null) }}>Convert</button></section></div>}
