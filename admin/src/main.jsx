  ...
  const renderFinanceHistory = (candidate) => {
    ...
    const invoiceRows = (candidate.invoices || []).map((invoice) => {
      ...
      return <div className="finance-history-row finance-invoice-history-row" key={`invoice-${invoice.id}`}>
        ...
        <small>{invoice.description || invoice.invoice_number}<br /><span className="finance-status-line"><span className={`finance-invoice-status status-${invoice.status} ${proof ? `proof-${proof.status}` : ''}`}>{status}</span><button className="finance-inline-action" onClick={() => openInvoiceForInvoice(candidate, invoice)}>Invoice</button>{proof && <button className="finance-inline-action" onClick={() => previewFinanceProof(proof)}>Preview</button>}{proof?.status === 'pending' && <button className="finance-inline-action finance-inline-accept" onClick={() => reviewProof(proof, 'accepted')}>Accept</button>}</span></small>
      </div>
    })
    const paymentRows = (candidate.payments || []).filter((payment) => !invoicePaymentIds.has(payment.id)).map((payment, paymentIndex) => <div className="finance-history-row" key={payment.id}>
      <span>{new Date(`${payment.payment_date}T00:00:00`).toLocaleDateString('id-ID')}</span>
