...
function App() {
  ...
  return (
    <main>
      ...
      {selectedFinanceCandidate && financeForm && <div className="modal-backdrop" role="presentation" onClick={() => setSelectedFinanceCandidate(null)}><section className="modal finance-modal" role="dialog" aria-modal="true" aria-labelledby="finance-title" onClick={(event) => event.stopPropagation()}><div className="modal-header"><div><span className="eyebrow">KEUANGAN KOLEKTIF</span><h2 id="finance-title">{selectedFinanceCandidate.full_name || 'Nama belum diisi'}</h2><p>{selectedFinanceCandidate.candidate_id}</p></div><button className="icon-button" aria-label="Tutup keuangan" title="Tutup" onClick={() => setSelectedFinanceCandidate(null)}><Icon name="close" /></button></div><div className="finance-fields"><label><span>Jasa paspor</span><input type="number" min="0" value={financeForm.passport_fee} onChange={(event) => setFinanceForm({ ...financeForm, passport_fee: event.target.value })} /></label><label><span>Jasa visa</span><input type="number" min="0" value={financeForm.visa_fee} onChange={(event) => setFinanceForm({ ...financeForm, visa_fee: event.target.value })} /></label><label><span>Biaya keberangkatan</span><input type="number" min="0" value={financeForm.departure_fee} onChange={(event) => setFinanceForm({ ...financeForm, departure_fee: event.target.value })} /></label></div><button className="collective-convert" onClick={saveFinance}>Simpan biaya</button><div className="finance-payment-section"><h3>Tambah cicilan</h3><div className="finance-fields"><label><span>Nominal</span><input type="number" min="1" value={paymentForm.amount} onChange={(event) => setPaymentForm({ ...paymentForm, amount: event.target.value })} /></label><label><span>Tanggal</span><input type="date" value={paymentForm.payment_date} onChange={(event) => setPaymentForm({ ...paymentForm, payment_date: event.target.value })} /></label><label className="finance-note"><span>Catatan</span><input value={paymentForm.note} onChange={(event) => setPaymentForm({ ...paymentForm, note: event.target.value })} /></label></div><button className="documents-button" disabled={!paymentForm.amount} onClick={addPayment}>Catat cicilan</button></div><div className="finance-history"><h3>Riwayat pembayaran</h3>{selectedFinanceCandidate.payments.length ? selectedFinanceCandidate.payments.map((payment) => <div className="finance-history-row" key={payment.id}><span>{new Date(`${payment.payment_date}T00:00:00`).toLocaleDateString('id-ID')}</span><strong>{formatCurrency(payment.amount)}</strong><small>{payment.note || 'Tanpa catatan'}</small></div>) : <p>Belum ada pembayaran.</p>}</div></section></div>}
