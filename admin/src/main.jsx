...
function App() {
  ...
  const renderFinanceHistory = (candidate) => {
    ...
    const invoiceRows = (candidate.invoices || []).map((invoice) => {
  ...
  const openInvoice = (candidate, payment, paymentIndex) => {
    ...
    setSelectedInvoice({
      candidate,
      payment,
      invoiceNumber: `INV-${payment.payment_date?.replaceAll('-', '') || 'DRAFT'}-${String(payment.id || paymentIndex + 1).slice(-6)}`,
  ...
  return (
    <main>
      ...
      {selectedInvoice && <div className="modal-backdrop invoice-backdrop" role="presentation" onClick={() => setSelectedInvoice(null)}><section className="modal invoice-modal invoice-preview-card" role="dialog" aria-modal="true" aria-labelledby="invoice-title" onClick={(event) => event.stopPropagation()}><div className="invoice-preview-heading"><div><span className="invoice-kicker">PREVIEW INVOICE</span><h2 id="invoice-title">Invoice cicilan</h2></div><button className="icon-button no-print" aria-label="Tutup invoice" title="Tutup" onClick={() => setSelectedInvoice(null)}><Icon name="close" /></button></div><div className="invoice-preview-rows"><div><span>Nomor invoice</span><strong>{selectedInvoice.invoiceNumber || `INV-${String(selectedInvoice.installmentNumber).padStart(4, '0')}`}</strong></div><div><span>Jatuh tempo</span><strong>{formatDateDisplay(selectedInvoice.payment.payment_date)}</strong></div><div><span>Keterangan</span><strong>{selectedInvoice.payment.note || `Pembayaran ke-${selectedInvoice.installmentNumber}`}</strong></div><div className="invoice-preview-total"><span>Total tagihan</span><strong>{formatCurrency(selectedInvoice.payment.amount)}</strong></div></div><div className="invoice-actions no-print"><button className="collective-convert" onClick={() => window.print()}>Cetak invoice</button><button className="documents-button" onClick={() => setSelectedInvoice(null)}>Tutup</button></div></section></div>}
