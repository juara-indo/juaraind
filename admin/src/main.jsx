...
function App() {
  ...
  return (
    <main>
      <header><div><span className="eyebrow">PT. JUARA · ADMIN</span><h1 className="admin-title">Dokumen kandidat</h1><p className="candidate-count">{candidates.length} peserta</p></div><div className="header-actions"><a className="admin-app-link" href="https://email.juaraind.com/" target="_blank" rel="noopener noreferrer">Email PT. Juara</a><button className="icon-button reload-button" aria-label="Muat ulang peserta" title="Muat ulang peserta" onClick={loadCandidates}><img src="/reload.svg" alt="" /></button><button onClick={() => getSupabase()?.auth.signOut()}>Keluar</button></div></header>
